...
# def QuantileTransformer(data, n_quantiles = 10):
#     # Create a QuantileTransformer object with 10 quantiles
#     # Quantize the dataset
#     quantized_data = qt.fit_transform(data).squeeze()
...
mnist_model = tf.keras.models.Sequential([
    ...
    # activation_quantize_fn(a_bit=1),
    tf.keras.layers.Dense(128),
    # activation_quantize_fn(a_bit=1),
    weight_quantize_fn(w_bit=1),
    tf.keras.layers.Dense(10)
])
"""

#########  v5 add "activation_quantize_fn" and "weight_quantize_fn"  into dense ############

def activation_quantize_fn(x, qmin=-128, qmax=127):
    scale = (qmax - qmin) / 255.0
    x = tf.cast(x / scale + 0.5, tf.int32)
    x = tf.clip_by_value(x, qmin, qmax)
    x = tf.cast(x, tf.float32)
    x = x * scale
    return x

def weight_quantize_fn(w, w_bit=8):
    w_min = tf.reduce_min(w)
    w_max = tf.reduce_max(w)
    scale = (w_max - w_min) / (2 ** w_bit - 1)
    w = tf.cast(tf.round(w / scale), tf.int32)
...
# mnist_model = tf.keras.models.Sequential([
#     tf.keras.layers.Flatten(input_shape=(28, 28)),
#     tf.keras.layers.Dense(10, activation=activation_quantize_fn,
#                           kernel_regularizer=weight_quantize_fn)
# ])
...
for k in range(0, Iter):   # epochs
    ...
    for m in range(0, M):  # works
        # vec_arr = np.random.rand(7850, Datatr.len())
        for (batch, (images, labels)) in enumerate(Datatr[m].take(1)):
            ...
            vec = gradtovec(grads)
            bvec = vec
            X = vec.reshape(len(vec), 1)
            vec = quantd(vec, mgr[m, :], b)  # quantization

            # vec = quandequan(X, len(vec)) # quantization
