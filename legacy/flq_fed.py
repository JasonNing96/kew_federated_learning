...
def run_one(mode, args):
    ...
    for k in range(args.iters):
        ...
        for m in range(M):
            ...
            if mode == "flq_bin":
                # 上行二值 + 懒惰
                vec_q, e_m = bin_relative_quant(gvec, mgr[m])
                e[m] = float(e_m)
                ...
                # 低比特相对量化 + 懒惰（典型 b_up=8）
                vec_q = quantd(gvec, mgr[m], b=args.b_up)
                diff = vec_q - gvec
                e[m] = float(diff @ diff)
        ...
        # 聚合与更新：对被触发客户端求平均，避免步长随参与数波动
        sel_cnt = float(sel_mask.sum())
        if sel_cnt > 0:
            dsa = dL[sel_mask].sum(axis=0) / sel_cnt
        ...
        # 统计比特：下行按 bit_down * nv * M；上行按 bit_up * nv * sel_cnt
        if mode in ["flq_bin", "flq_lowbit"]:
            bit_down = 8 * nv * M           # 下行8-bit参数广播
        elif mode == "laq8":
        ...
        if mode == "flq_bin":
            ...
            bit_up = args.b_up * nv * sel_cnt    # 典型 b_up=8
        elif mode == "laq8":
            bit_up = 8 * nv * M
        else:
            bit_up = 32 * nv * M
    ...
    if pd is not None:
        ...
        fn = f"./results/{args.out_prefix}_{args.dataset}_{mode}.xlsx"
        with pd.ExcelWriter(fn) as xw:
            df_curve.to_excel(xw, sheet_name=f"curve_{mode}", index=False)
            # 图2：二值序列
            import numpy as _np
            pd.DataFrame({"comm": _np.arange(len(bin_series)),
                          "bit": _np.array(bin_series, dtype=int)}
...
def main():
    ...
    ap.add_argument("--b_up", type=int, default=1, help="uplink bitwidth for FLQ; bin=1, lowbit=8等")
    ...
    ap.add_argument("--down_laq8", type=int, default=32)  # LAQ 下行位宽统计口径
