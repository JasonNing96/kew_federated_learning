...
def plot_flq(csv_path: Path, out_dir: Path) -> None:
    """
    从 CSV 读取数据，绘制若干典型曲线：
    ...
    if round_col is None:
        raise RuntimeError("CSV 中未找到轮次列（round/current_round/completed_rounds）。")

    x = df[round_col]

    # 1) 精度 / mAP 曲线
    metric_cols = [c for c in df.columns if any(k in c.lower() for k in ["map", "acc", "accuracy"])]
    for col in metric_cols:
        ...
        plt.ylabel(col)
        plt.grid(True)
        plt.title(f"{col} vs. Round")
        fig_path = out_dir / f"{col.replace('.', '_')}_vs_round.png"
    ...
    if has_bits:
        ...
        plt.grid(True)
        plt.legend()
        plt.title("Communication Bits per Round")
        fig_path = out_dir / "bits_vs_round.png"
        plt.savefig(fig_path, bbox_inches="tight")
        plt.close()
        print(f"[plot] 保存图像: {fig_path}")
    else:
    ...
    if has_bits and metric_cols:
        ...
        for col in metric_cols:
            plt.figure()
            plt.plot(df["bits_cumsum"], df[col], marker="o")
            plt.xlabel("Cumulative Bits")
            plt.ylabel(col)
            plt.grid(True)
            plt.title(f"{col} vs. Cumulative Bits")
            fig_path = out_dir / f"{col.replace('.', '_')}_vs_cum_bits.png"
            plt.savefig(fig_path, bbox_inches="tight")
            plt.close()
            print(f"[plot] 保存图像: {fig_path}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p_plot.add_argument("--csv", type=str, required=True, help="训练阶段生成的 CSV 文件路径")
    p_plot.add_argument(
        "--out-dir",
        type=str,
    ...
    if args.command == "plot":
        plot_flq(Path(args.csv), Path(args.out_dir))
        return 0
