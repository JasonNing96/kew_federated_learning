...
def fedavg_aggregate(updates: List[Dict], weights: List[int]) -> Dict:
    """
    FedAvg 聚合算法（加权平均）
    
    Args:
        updates: 客户端更新列表 [{state_dict}, ...]
        weights: 客户端样本数列表 [n1, n2, ...]
    
    Returns:
        aggregated: 聚合后的 state_dict
    """
    if not updates:
    ...
    for key in updates[0].keys():
        aggregated[key] = sum(
            w * update[key] for w, update in zip(normalized_weights, updates)
        )
